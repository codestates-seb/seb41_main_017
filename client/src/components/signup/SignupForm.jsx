...
function SignForm() {
  ...
  const handleSignupBtn = (e) => {
    ...
    axios
      .post(`${process.env.REACT_APP_URL}/users/signup`, JSON.stringify(reqbody), { headers })
      ...
      .catch((err) => {
        ...
        if ((termsChecked === false && privacyChecked === false) || response === 405) {
    ...
    <Page>
      <IdBlock>
        ...
        <div className="error_box">
          {signupId.length < 6 && 0 < signupId.length ? <div className="error_text">6자 이상 16자 이하의 글자 수를 입력해주세요 </div> : null}
        </div>
        ...
        <div className="error_box">
          {signupPassword && !regexp.test(signupPassword) ? (
            <div className="error_text">숫자, 알파벳, 특수문자(!@#$%^&*) 포함 8자 이상 20자 이하로 입력해주세요</div>
          ) : null}
        </div>
        <div className="error_box">
          {checkPassword && signupPassword !== checkPassword ? <div className="error_text2">비밀번호가 일치하지 않습니다</div> : null}
        </div>

        <div className="input_cotainer">
          <BasicInput setValue={setName} star={"*"} label={"이름"} width={"100%"} type={"text"} placeholder={"이름을 입력해주세요"}></BasicInput>
        </div>

        <div className="error_box">
          {name && name.length > 0 && !nameCheck.test(name) ? (
            <div className="error_text">올바르지 않은 이름입니다. (공백,특수문자,숫자는 사용불가합니다.)</div>
          ) : null}
        </div>
        ...
        <div className="error_box">
          {signupEmail && signupEmail.length > 0 && !emailCheck.test(signupEmail) ? (
            <div className="error_text">올바르지 않은 이메일 입니다. [공백, 특수문자(!@#$%^&*-_)는 사용불가합니다.]</div>
          ) : null}
        </div>
        ...
        <div className="error_box">
          {phoneNum && phoneNum.length > 0 && !phoneCheck.test(phoneNum) ? (
            <div className="error_text">올바르지 않은 번호 입니다. [ex 010-0000-0000]</div>
          ) : null}
        </div>
        <div className="input_cotainer">
          <div className="input_box">
            ...
            <BasicInput setValue={setDetailAddress} type={"text"} placeholder={"상세주소를 입력해주세요"} width={"100%"} />
          </div>
        ...
        <div className="input_birth">
          <BasicInput setValue={setYear} label={"생년월일"} star={"*"} type={"text"} width={"90%"} placeholder={"YYYY"}></BasicInput>
          <BasicInput setValue={setMonth} type={"text"} placeholder={"MM "} width={"90%"}></BasicInput>
          <BasicInput setValue={setDay} type={"text"} placeholder={"DD"} width={"90%"}></BasicInput>
        </div>
        ...
        <ButtonWrapper onClick={handleSignupBtn}>
          <BasicButton children={"가입하기"} font={"20"} radius={"5"} p_height={"14"} p_width={"150"} />
        </ButtonWrapper>
      </IdBlock>
